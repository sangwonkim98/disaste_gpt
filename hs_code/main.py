        ...
        with gr.Blocks(title=f"{PROJECT_NAME} {VERSION}", theme=GRADIO_THEME) as demo:
            ...
            with gr.Row():
                ...
                with gr.Column(scale=5):
                    ...
                    with gr.Row():
                        reasoning_mode = gr.Checkbox(label="🧠 Reasoning 모드", value=True, visible=True)
                        enable_reasoning = gr.Checkbox(label="🤔 추론과정 표시", value=True, visible=True)
                        agent_mode = gr.Checkbox(label="🤖 Agent 모드", value=True, visible=True)
                        enable_rag = gr.Checkbox(label="📚 문서 검색", value=True, visible=True)
            ...
            def user_input(user_message, history, reasoning_mode, enable_reasoning, agent_mode, enable_rag, selected_pdf, uploaded_file):
                ...
                # [Format Conversion] Chatbot(messages) -> Manager(tuples)
                # Gradio Chatbot이 messages 포맷(list of dicts)을 사용하므로, 
                # 내부 로직(tuples)에 맞게 변환이 필요함.
                history_tuples = []
                if history:
                    current_user = None
                    for msg in history:
                        if isinstance(msg, dict):
                            if msg.get('role') == 'user':
                                ...
                                if current_user is not None:
                                    history_tuples.append([current_user, msg.get('content')])
                                    current_user = None
                        elif isinstance(msg, (list, tuple)) and len(msg) == 2:
                            # 만약 이미 튜플 형태라면 그대로 사용 (호환성)
                ...
                # 빈 메시지 처리
                if not user_message:
                    # 빈 메시지일 경우 입력 history 포맷에 맞춰 반환
                ...
                for result in self.chat_manager.process_message(
                    ...
                    new_history_tuples, raw_doc_info = result
                    
                    # [Format Conversion] Manager(tuples) -> Chatbot(messages)
                    # 내부 로직 결과(tuples)를 다시 Chatbot용 messages 포맷으로 변환
                    new_history_dicts = []
                    for u, a in new_history_tuples:
                        new_history_dicts.append({"role": "user", "content": u})
                        new_history_dicts.append({"role": "assistant", "content": a})
                    
                    # [UI Feedback] 파일 업로드 시 응답 상단에 파일명 표시
                    if file_name_display and new_history_dicts:
                        last_idx = -1
                        last_content = new_history_dicts[last_idx]['content']
                        upload_msg = f"📂 **파일 업로드됨:** `{file_name_display}` (분석 중...)\n\n"
                        if not last_content.startswith("📂 **파일 업로드됨"):
                            new_history_dicts[last_idx]['content'] = upload_msg + last_content
                    
                    if enable_reasoning and reasoning_mode:
                        panel_thinking = ""
                        if new_history_dicts:
                            last_response = new_history_dicts[-1]['content'] or ""
                            think_marker = "🤔 **[추론 과정]**"
                        ...
                        status_info = ""
                        if new_history_dicts:
                            if "📡 API 호출 상태:" in new_history_dicts[-1]['content']:
                        ...
                        yield new_history_dicts, thinking_display, raw_doc_info, "", submit_btn_enabled, None
                    else:
                        yield new_history_dicts, "", raw_doc_info, "", submit_btn_enabled, None
                
                submit_btn_reactivated = gr.Button(interactive=True)
                
                if 'new_history_dicts' in locals() and new_history_dicts:
                    last_content = new_history_dicts[-1]['content']
                    if last_content:
                        if last_content.strip() and not last_content.endswith("🔄"):
                            yield new_history_dicts, thinking_display if (enable_reasoning and reasoning_mode) else "", raw_doc_info, "", submit_btn_reactivated, None
                        else:
                            new_history_dicts[-1]['content'] = last_content + "\n\n⚠️ 응답이 완료되었습니다."
                            yield new_history_dicts, thinking_display if (enable_reasoning and reasoning_mode) else "", raw_doc_info, "", submit_btn_reactivated, None
                    else:
                        # 응답이 비어있는 경우
                        error_msg = {"role": "assistant", "content": "❌ 응답 생성 중 오류가 발생했습니다."}
                        # 마지막 사용자 메시지에 대한 응답이 없으면 추가
                        if new_history_dicts[-1]['role'] == 'user':
                            new_history_dicts.append(error_msg)
                        yield new_history_dicts, "", "오류 발생", "", submit_btn_reactivated, None
                else:
                    # 히스토리가 아예 없는 경우 (초기 에러)
                    # 입력된 메시지는 유지해야 함
                    error_history = history + [{"role": "user", "content": user_message}, {"role": "assistant", "content": "❌ 응답을 생성할 수 없습니다."}]
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(f"🚀 Launching Gradio App (Share Mode: {share_mode})")
        app.launch(host=args.host, port=args.port, share=share_mode)
