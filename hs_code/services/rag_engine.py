...
class AdvancedRAGSystem:
    ...
    def __init__(self):
        ...
        if self.use_gpu:
            ...
            logger.info("GPU FAISS 사용 중지 - CPU 모드로 실행")

        # 임베딩 디바이스 설정 (config에서 가져옴, 기본값 cpu)
        # vLLM 서버가 GPU 메모리 대부분을 사용하므로 임베딩은 CPU 권장
        self.device = EMBEDDING_DEVICE
        logger.info(f"💻 임베딩 디바이스: {self.device} (EMBEDDING_DEVICE 설정)")
        ...
        if CROSS_ENCODER_RERANKING_ENABLED and RERANKER_AVAILABLE:
            try:
                ...
                # Pydantic v2 호환성 수정: model_rebuild() 호출
                try:
                    FlashrankRerank.model_rebuild()
                except Exception:
                    pass  # 이미 빌드되었거나 필요 없는 경우
    ...
    def create_documents_from_pdf(self, pdf_path: str) -> List[Document]:
        ...
        # [FIX] 페이지별 텍스트 추출 및 청킹 (페이지 번호 보존)
        full_text = self.ocr_processor.extract_pdf_text(pdf_path)
        ...
        # split 결과: [서문, '1', 1페이지텍스트, '2', 2페이지텍스트, ...]
        # 첫 번째 요소는 서문(페이지 번호 없음)일 수 있음
        
        current_chunk_id = 0
        
        # 0번 인덱스(서문) 처리
        if page_splits[0].strip():
            chunks = self.text_splitter.split_text(page_splits[0])
            for chunk in chunks:
                ...
                documents.append(Document(
                    ...
                    metadata={
                        ...
                        "page_num": 0, # 서문은 0페이지로 간주
                        "chunk_length": len(chunk),
                    }
                ))
                current_chunk_id += 1
        
        # 페이지별 처리 (1번 인덱스부터는 번호, 텍스트 쌍)
        for i in range(1, len(page_splits), 2):
            if i + 1 >= len(page_splits): break
            ...
            for chunk in chunks:
                ...
                documents.append(Document(
                    ...
                    metadata={
                        ...
                        "pdf_name": Path(pdf_path).name,
                        "chunk_id": current_chunk_id,
                        "page_num": page_num, # 정확한 페이지 번호
                        "chunk_length": len(chunk),
                    }
                ))
                current_chunk_id += 1

        logger.info(f"문서 청킹 완료: {len(documents)}개 (페이지 정보 포함), {time.time()-start:.2f}s")
        return documents
    ...
    def search(self, query: str, selected_pdf_path: str = None, top_k: int = TOP_K_RESULTS) -> List[Dict]:
        ...
        과정:
        1. Query Rewriting: 사용자의 질문을 검색에 최적화된 형태로 재작성 (LLM 활용)
        2. Candidate Retrieval: 벡터 DB(FAISS)에서 유사한 문서 청크를 1차 검색
        3. Reranking: 검색된 후보군을 Cross-Encoder 등을 사용해 정밀하게 재순위화
        4. Diversity Constraint: 한 문서에 너무 편중되지 않게 다양성 필터링
        
        Args:
            query: 사용자 질문
            selected_pdf_path: 특정 PDF에서만 검색할 경우 경로 (옵션)
            top_k: 최종 반환할 문서 개수
        """
        start = time.time()
        
        # 1. 쿼리 재작성 (Query Rewriting)
        # "강풍 주의보 기준?" -> "강풍주의보 발령 기준 풍속 매뉴얼" 등으로 변환
    ...
    def format_search_results(self, results: List[Dict], selected_pdf_path: str = None) -> str:
        ...
        for i, r in enumerate(results, 1):
            ...
            # [FIX] 청크 ID 대신 페이지 번호 표시 (UX 개선)
            formatted += f"**페이지:** {page_num}쪽 | **관련도:** {r['similarity_score']:.4f}\n\n"
            formatted += f"**내용:**\n```\n{r['text']}\n```\n\n"
            formatted += "---\n\n"
