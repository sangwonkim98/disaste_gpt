...
2. Do NOT include any explanations, Markdown formatting (like ```json), or comments.
3. If the Date or Region is not explicitly stated, infer it from the context (e.g., "서울 기온", "오늘(31일)").
4. If inference is impossible, mark status as "incomplete".
  ...
  "sections": [
    {
      "section_id": "1",
      "title": "기상 전망",
      "original_text": "오늘 서울 날씨는...",
      "search_query": "서울 기상 전망",
      "requires_tools": true
    }
  ]
}
...
Case 2: Missing Info
{
  ...
  "clarification_question": "어느 지역의 보고서를 작성하시겠습니까?"
}
"""
...
"""
        ...
        for attempt in range(3):
            try:
                ...
                # [DEBUG] 입력 프롬프트 저장
                try:
                    with open("hs_code/debug_parser.log", "a", encoding="utf-8") as f:
                ...
                # [DEBUG] Response 객체 구조 확인을 위해 덤프
                try:
                    with open("hs_code/debug_parser.log", "a", encoding="utf-8") as f:
                        f.write(f"\n[Raw Response Dump]\n{str(response)}\n")
                except: pass
                ...
                if not response.choices:
                    ...
                    logger.warning("Response.choices is empty")
                    continue
                    
                message = response.choices[0].message
                content = message.content
                
                # [FIX] content가 비어있고 reasoning_content에 내용이 있다면 대체 사용
                if not content and hasattr(message, 'reasoning_content') and message.reasoning_content:
                    logger.info("ℹ️ content is empty, using reasoning_content instead.")
...
"""
            ...
            try:
                response = self.client.generate_response(
                    ...
                    enable_thinking=False, # [최적화] 속도 향상을 위해 Thinking 비활성화
                ...
                # [안전장치] content가 여전히 None이면 빈 문자열로 변환
    ...
    def export_to_docx(self, structure: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if self.template_path.exists():
                ...
                except ValueError: # 스타일 에러 방지
                    doc.add_paragraph(structure.get('document_type', 'Disaster Report')).bold = True
            ...
            for section in structure.get("sections", []):
                ...
                if not inserted:
                    try:
                        ...
                        p.runs[0].bold = True
                        p.runs[0].font.size = Pt(14) # 제목 크기 흉내
